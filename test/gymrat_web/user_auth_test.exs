defmodule GymratWeb.UserAuthTest do
  use GymratWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Gymrat.Accounts
  alias Gymrat.Accounts.Scope
  alias GymratWeb.UserAuth

  import Gymrat.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, GymratWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      GymratWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        GymratWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
